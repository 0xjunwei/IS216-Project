...
let unsub = null;
onMounted(() => {
  unsub = auth.onAuthStateChanged((uid) => {
    user.value = uid;
  });
});
...
function loadPantryFromStorage() {
  ...
  if (savedPantry) {
    try {
      pantry.value = JSON.parse(savedPantry);
      // Update nextId to avoid conflicts
      if (pantry.value.length > 0) {
        nextId = Math.max(...pantry.value.map(item => item.id)) + 1;
      }
    } catch (e) {
      console.error('Error loading pantry from storage:', e);
    }
  }
}

function savePantryToStorage() {
  localStorage.setItem('pantry', JSON.stringify(pantry.value));
}
...
function addItem() {
  ...
  savePantryToStorage(); // Save to localStorage
}
...
async function retrievePantry() {
  ...
  loadPantryFromStorage();
  console.log("Retrieved pantry from localStorage:", pantry.value);
}

// Test function - now saves to localStorage instead of Firestore
async function testSetPantryToFirestore() {
  // Example pantry data
  const testPantry = [
    { id: 1, name: "chicken breast", qty: 2, unit: "pcs", expiry: "2025-10-20" },
    { id: 2, name: "milk", qty: 1, unit: "L", expiry: "2025-10-10" },
    { id: 3, name: "broccoli", qty: 3, unit: "pcs", expiry: "2025-10-09" },
  ];

  try {
    pantry.value = testPantry;
    savePantryToStorage();
    console.log("Test pantry written to localStorage!");
  } catch (err) {
